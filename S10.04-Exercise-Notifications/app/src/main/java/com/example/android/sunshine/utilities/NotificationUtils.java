...
//          COMPLETED (3) Create an Intent with the proper URI to start the DetailActivity
            Long date = SunshineDateUtils.getNormalizedUtcDateForToday();
            Intent weatherNotificationIntent = new Intent(context, DetailActivity.class);
            Uri uriForDateClicked = WeatherContract.WeatherEntry.buildWeatherUriWithDate(date);
            weatherNotificationIntent.setData(uriForDateClicked);

//          COMPLETED (4) Use TaskStackBuilder to create the proper PendingIntent
            TaskStackBuilder taskStackBuilder = TaskStackBuilder.create(context);
            taskStackBuilder
                    .addParentStack(DetailActivity.class)
                    .addNextIntent(weatherNotificationIntent);
            PendingIntent weatherNotificationPendingIntent = taskStackBuilder.getPendingIntent(
                    NOTIFICATION_ID,
                    PendingIntent.FLAG_UPDATE_CURRENT);
//          COMPLETED (5) Set the content Intent of the NotificationBuilder
            notificationBuilder.setContentIntent(weatherNotificationPendingIntent);
...
//          COMPLETED (8) Save the time at which the notification occurred using SunshinePreferences
            SunshinePreferences.saveLastNotificationTime(context, date);
        }
